...
\begin{document}
    ...
    In Bosman's and Olver's paper ``Exploring the Tractability of the Capped Hose Model'', a dynamic program is suggested for these cirucmstances.
    ...
    \[
        C[i, j, h(i), h(j)] = \begin{cases}
                                  ...
                                  \min_{h(k) \in V_G} C[i, k, h(i), h(k)] + C[k, j, h(k), h(j)] - b_k d(v_k, h(k)) &\text{otherwise}\\
